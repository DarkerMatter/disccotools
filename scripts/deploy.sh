#!/usr/bin/env bash
# scripts/deploy.sh deploys disccotools to tools.dimitri.one
#
...
#   4. Deploy the SPA + Pages Functions to Cloudflare Pages.
#   5. Smoke-test https://tools.dimitri.one/api/health.
#
# Prerequisites (one-time, manual):
#   - wrangler login
#   - wrangler d1 create disccotools                  -> paste id into wrangler.toml
#   - wrangler r2 bucket create disccotools
...
#   - Upload favicon to R2 at key static/disccotools.png
#       wrangler r2 object put disccotools/static/disccotools.png --file=./favicon.png
...
# Resolve pnpm. Prefer one on PATH, fall back to corepack (bundled with Node 20+).
...
echo "==> [4/5] Deploying SPA + Pages Functions to Cloudflare Pages"
(
